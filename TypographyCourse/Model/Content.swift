...
let welcome = Page(
    ...
    challengeHelp: "This challenge aims to introduce you to the playground area and demonstrate its intractability. Click the blue »Got it« button to progress.",
    elements: [
        PageText("This course introduces you to some fundamental rules of typography. In about 15 minutes you can boost your skills on how to work with text and acquire some solid practical typographic knowledge. After finishing, there's still so much more to discover, so let's start right away."),
        ...
        PageText("If you ever get stuck at a challenge, tap the blue question mark in the upper right corner to get help.", topSpacing: true),
...
let fonts = Page(
    ...
    challengeHelp: "In this challenge, you have to select the right font combination with the three pickers below. If you tap a picker, the view shows you with a blue highlight, which text is going to be edited. Remember to choose a consistent type face and think about which font category is best suited for screens.",
    elements: [
...
let hierarchy = Page(
    ...
    elements: [
        PageText("Good typography lets readers easily understand the semantic of different paragraphs in a layout, so that the type of each paragraph is obvious, e.g. Headline or Body text. Hierarchy allows achieving that desired effect."),
        ...
        PageText("Fonts have different weights. Thin, Regular or Semibold just to name a few. In addition to working with different font sizes, you can also use these weights to create hierarchy and empathize different paragraphs. Whenever you use more than one style of a font, make sure to skip a weight, otherwise, the difference between those two weights is too subtle. You can for example choose a combination of Regular and Semibold, if there is a medium in between.", bottomSpacing: true),
        PageCustomView(.hierarchyContentCustomView, topSpacing: true, bottomSpacing: true),
        PageText("Above, you can see an example of a very simple hierarchy, similar to the one used in this app. It's achieved through a combination of font sizes and weights. For your projects, think about the different kinds of paragraphs you have, create a hierarchy system and stick to it."),
        ...
        PageHeadline("Summary", topSpacing: true),
        PageText("Emphasize different paragraphs with a consistent hierarchy system. When working with font stiles, skip a weight for better differentiation."),
    ]
)
...
let alignment = Page(
    ...
    challengeHelp: "In this challenge, you have to select the correct paragraph alignments with the three pickers below. If you tap a picker, the view shows you with a blue highlight, which text is going to be edited. Think about which alignment option is best suited for Left-to-Right languages, like English in this case.",
    elements: [
        PageText("Your text alignment choice has a great influence on the readability of a text. Most of the time, especially for body text, the best option is to left align your text."),
        PageHeadline("Natural reading flow", topSpacing: true),
        PageText("Supporting the natural language flow with your alignment improves readability. So in left to right languages like English or German, align your text to the left. There are other languages, in which text is read from right to left. If you are designing for such a language, choose the right text align option."),
        PageHeadline("Justified text", topSpacing: true),
        PageText("You should avoid justified alignment, as the inconsistent spacing between the words makes reading more demanding. We don't care that I may look a bit sleeker sometimes, because readability is the priority."),
        PageTask("There's a newspaper article on the right, play around with the different alignment options and then choose for each paragraph the best fitting one.", topSpacing: true),
        PageHeadline("Summary", topSpacing: true),
        PageText("In left to right languages, left align your text — especially body text. Other text can be centered in some cases, but be sure to stick to a consistent, simple system. When in doubt, left align all texts."),
    ]
)
...
let detail = Page(
    ...
    challengeHelp: "Use the first slider below to select a font size of or around 60 pt. Then use the second slider to adjust the tracking until you have found a value, that lets the word appear more spaced out. You'll need to increase the value.",
    elements: [
        ...
        PageTask("Set the font size to about 60 pt. Then play around with the tracking and select a value that makes the word appear a bit more spaced out.", topSpacing: true),
        PageHeadline("Summary", topSpacing: true),
